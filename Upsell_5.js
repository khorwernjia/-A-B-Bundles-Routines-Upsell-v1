(() => {
    ...
    const SERUM_2ML_VARIANT_IDS = new Set([48412528672999, 50695797080295]);
    ...
    function isSerum2mlLine(item) {
      return (
        HANDLE_ALIASES.serum.includes(normaliseHandle(item.handle)) &&
        SERUM_2ML_VARIANT_IDS.has(Number(item.variant_id))
      );
    }

    function hasSerum2mlInCart(items) {
      return items.some(isSerum2mlLine);
    }
    ...
    function buildEligibleCards(cart) {
      ...
      const hasSerum2ml = hasSerum2mlInCart(items);
      ...
      const combos = [
        ...
        {
          condition: hasSerum2ml && !hasMac && !hasStarterMac,
          id: "serum2ml-mac",
          triggerHandle: PRODUCT_HANDLES.serum,
          missingHandle: PRODUCT_HANDLES.mac,
        },
        {
          condition: hasSerum && !hasSerum2ml && !hasMac && !hasStarterMac,
          id: "serum-mac",
          triggerHandle: PRODUCT_HANDLES.serum,
          missingHandle: PRODUCT_HANDLES.mac,
        },
      ...
      let resolvedTriggerHandle = first.triggerHandle;
      let serum2mlLine = null;
      if (first.id === "serum2ml-mac") {
        serum2mlLine = items.find(isSerum2mlLine);
        if (serum2mlLine) resolvedTriggerHandle = normaliseHandle(serum2mlLine.handle);
      }

      return Promise.all([
        fetchProduct(resolvedTriggerHandle),
        ...
        } else if (first.id === "serum2ml-mac") {
          if (serum2mlLine) forceTriggerVariantId = Number(serum2mlLine.variant_id);
        }
